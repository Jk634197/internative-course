...
router.post(
  "/signUp",
  [
    body("email").isEmail().withMessage("please pass valid email id"),
    body("password").isString().withMessage("please pass valid password"),
    body("first").isString().withMessage("please pass valid first name"),
    body("last").isString().withMessage("please pass valid last name"),
    body("mobileNo").isMobilePhone().withMessage("please pass valid mobile no"),
  ],
  checkErr,
  async (req, res, next) => {
    try {
      ...
      return res.status(200).json({
        issuccess: true,
        data: { acknowledgement: true, data: createUser },
...
router.post(
  "/resend-otp",
  [body("id").notEmpty().isEmail().withMessage("please provide email id")],
  checkErr,
  async (req, res, next) => {
    try {
      const { id } = req.body;
    ...
    const userList = await userSchema.aggregate([
      ...
      },
      {
        $lookup:{
          from:'usercourses',
          let:{userId:'$_id'},
          pipeline:[{$match:{$expr:{$and:[{$eq:['$userId','$$userId']},{$or:[{$eq:['$status',1]},{$eq:['$status',2]}]}]}}},{
            $lookup:{
              from:'courses',
              localField:'courseId',
              foreignField:'_id',
